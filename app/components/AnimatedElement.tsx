...
export const AnimatedElement = <T extends ElementType>({
  ...
  const variants = useMemo(
    () => ({
      hidden: { opacity: 0, filter: `blur(${blur}px)`, ...directionOffset(fadeDirection, offsetPx) },
      visible: { opacity: 1, x: 0, y: 0, filter: "blur(0px)", transition: { duration, delay, ease: [0.2, 0.65, 0.3, 0.9] } },
    }),
    [delay, duration, fadeDirection, offsetPx, blur]
  )
